...
void solve() {
    ...
    //intuition : let x = 10, y = 3, n = 45; y is a remainder. what if we found the larges perfect
    // multiple of x that is <= n? in our case it is 40. 40 % 10 = 0. so to get the K we need to add 
    // the remander back so it will be 40 + 3 = 43 => 43 % 10 = 3 which is equal to y. 
    // How to get the perfect multiple? simply you divide n by x => 45 / 10 = 4 , and multiply the 
    // the result by x to get the perfect multiple. But there is a catch. consider the test case
    // x = 10, y = 7, n = 25, if we divided by n alone we will get the final k as 27 which is > 25,
    // so to avoid this we subtract by y to have a "clean-start"
}
